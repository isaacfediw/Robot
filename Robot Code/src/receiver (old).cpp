...
void checkData() {
  ...
  if (status & SYS_STATUS_RXFCG_BIT_MASK) {
        ...
        if (rx_data[4] == 0xFF) {
          ...
          /*if (!received_t4) {
            if (STATE != STATES::FINISH) {
              ...
              Serial.printf("Sent 0xAA (Because we didn't get t4) Tries: %d\n", tries);
          ...
          if (STATE != STATES::FINISH) {
            //Serial.printf("Current Distance: %.2f, ", current_distance);
            //Serial.printf("Previous Distance: %.2f\n", prev_distance);
    ...
    Serial.printf("[ERROR] Interrupt fired, but status is 0x%08X\n", status);
    if (status & SYS_STATUS_RXFCE_BIT_MASK) Serial.println(" -> RXFCE: Frame Check (CRC) Error");
    if (status & SYS_STATUS_RXOVRR_BIT_MASK) Serial.println(" -> RXOVRR: Receiver Overrun");
    if (status & SYS_STATUS_RXFTO_BIT_MASK) Serial.println(" -> RXFTO: Frame Wait Timeout");
    if (status & SYS_STATUS_RXFSL_BIT_MASK) Serial.println(" -> RXFSL: Sync Loss / Preamble Error");
...
bool respond(uint32_t dest_address) {
  delay(COMM_DELAY); // give the transmitter time to re-enable its receiver
